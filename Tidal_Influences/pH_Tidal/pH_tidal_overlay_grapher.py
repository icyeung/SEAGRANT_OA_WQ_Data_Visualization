...
def pH_tidal_overlay_grapher(ph_file, tide_file_location, tide_location, start_date, end_date, graph_title, graph_save_name):

    # Obtains location of pH_Tidal subfolder
    __location__ = os.path.realpath(os.path.join(os.getcwd(), os.path.dirname(__file__)))

    # Opens pH data as dataframe
    pH_section_data = pd.read_csv(os.path.join(__location__, ph_file))
    ...
    # Gets pH data and formats pH dates into datetime format and also removes year, but latter function is not necessary
    pH_data = pH_section_data["pH"]
    pH_date = pH_section_data["Date"]
    pH_date_revised = []
    for date in pH_date:
    ...
    # Graphs pH
    p1 = ax1.plot(pH_date_revised, pH_data, color = "b", linestyle = 'solid', label = 'pH', linewidth=0.75)
...
# To use this program, please input a new line containing the ph file name, tide file location, tide location, first date of ph file, last date of ph file, graph title, and save name for thr graph
# The new line should be in this format (without quotations): "pH_tidal_overlay_grapher(ph file name, tide file location, tide location, ph start date, ph end date, graph title, save name for graph)"
...
# Oct 2022 pH vs Monument Beach Tide
pH_tidal_overlay_grapher("pH_2022_subsection_oct.csv", "\\Tidal_Influences\\NOAA_Tidal_HL_2022_Dennisport_GMT.csv", "Dennisport", "09-30-2022", "11-01-2022", "pH & High/Low Tidal Markers (2022)", "\\pH_tidal_overlay_oct_2022_Graph_No_Outliers.png")
