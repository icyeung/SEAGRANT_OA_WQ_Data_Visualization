...
def NERRS_mstl_grapher(file_location, date_start, date_end, trunc_date_start, trunc_date_end, data_save_name, seasonal_period, data_year):
    ...
    graph_bd_save_name = "NERRS_" + str(data_year) + "Metoxit_MSTL_Graph_SeasonalPeriod_" + str(seasonal_period)+ "z2.5_Breakdown1.png"
    ...
    remainder_cleaned = residual.copy()
    remainder_cleaned[remainder_cleaned > threshold] = np.nan
    remainder_cleaned[remainder_cleaned < -threshold] = np.nan

    remainder_interpolated = remainder_cleaned.interpolate(method='linear')

    cleaned_series = trend + seasonal["seasonal_12"] +seasonal["seasonal_708"] + remainder_interpolated
    
    plt.figure(figsize=(10, 6))
    plt.plot(not_flagged_data["Datetime_UTC"], not_flagged_data['Sal'], label='Original Salinity', marker = ".", color = 'blue')
    plt.plot(cleaned_series, label='Reconstructed after MSTL & Interpolation', marker = "+", color = 'red')
    plt.legend("upper right")
    plt.title('Time Series after MSTL and Interpolation')

    plt.savefig("test int.png")
