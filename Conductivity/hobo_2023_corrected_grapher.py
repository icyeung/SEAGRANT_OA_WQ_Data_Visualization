...
hobo_corrected_sal_list = hobo_data_corrected["Salinity_Value_Corrected"]
hobo_corrected_noyear = hobo_data_corrected["Datetime_noyear"]
hobo_corrected_noyear_list = []
for date in hobo_corrected_noyear:
    dt_date_no_year = dt.strptime(date, "%Y-%m-%d %H:%M:%S")
    hobo_corrected_noyear_list.append(dt_date_no_year)
...
p9 = ax1.plot(hobo_corrected_noyear_list, hobo_corrected_sal_list, color = "teal", linestyle = 'solid', label = 'HOBO Corrected', linewidth=0.75)
...
p3 = ax1.plot(bbc_d_datetime_noyear_list, bbc_d_sal_list, color = "lime", linestyle = 'solid', label = 'BBC PR1 2023 Deep', linewidth=0.75, marker = "o")
...
plt.title("HOBO #1 2022 (-3 Hours) (proxy for 2023) vs BBC 2023 PR1 vs Scallop Bay Marina [Corrected]", loc='center')
plt.grid(True)
plt.legend()
