...
def sami_hampel_filter(file_loc, year, window):
    ...
    axes[0].plot(original_sami_data['Date'], pco2_original, label='Original pCO2')
    ...
    axes[0].set_title(str(year) + ' Original pCO2 Data with Outliers')
    axes[0].legend()

    # Plot the filtered data in the second subplot
    axes[1].plot(original_sami_data['Date'], filtered_sami, label='Filtered pCO2')
    axes[1].set_title(str(year) + ' pCO2 Data Hampel Filter' + ' (Window Size = ' + str(window) + ')')
    axes[1].legend()


    my_path = os.path.dirname(os.path.abspath(__file__))
    graph_save_name = "pCO2_" + str(year) + "_Hampel_Graph_WindowSize_" + str(window)+ ".png"
    plt.savefig(my_path + '\\pCO2_Graphs\\Hampel_Filter\\' + graph_save_name)
...
'''
# Hampel Filter
# Sourced from https://stackoverflow.com/a/46940319
