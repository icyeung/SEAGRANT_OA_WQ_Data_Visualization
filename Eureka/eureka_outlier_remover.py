...
def eureka_outlier_remover(file_name, location):
    ...
    for index in range(1, len(NERRS_data)):
        salinity = NERRS_data.loc[index, "Salinity"]
    ...
    for index in range(1, len(NERRS_data)-1):
        print("--------------------------------")
        date_current = NERRS_data.loc[index, "Time (UTC)"]
        date_current_dt = dt.strptime(date_current, '%m-%d-%Y %H:%M:%S')
        salinity_current = NERRS_data.loc[index, "Salinity"]
    
        date_before = NERRS_data.loc[index-1, "Time (UTC)"]
        date_before_dt = dt.strptime(date_before, '%m-%d-%Y %H:%M:%S')
        salinity_before = NERRS_data.loc[index-1, "Salinity"]
        ...
        date_after = NERRS_data.loc[index+1, "Time (UTC)"]
        date_after_dt = dt.strptime(date_after, '%m-%d-%Y %H:%M:%S')
        salinity_after = NERRS_data.loc[index+1, "Salinity"]
    ...
    time_list = []
    for i in NERRS_data_new["Time (UTC)"]:
        datetime_obj = dt.strptime(i, '%m-%d-%Y %H:%M:%S')
        time_list.append(datetime_obj)

    # Graphing
    fig, ax1 = plt.subplots(figsize=(14,7))
    p1 = ax1.scatter(time_list, NERRS_data_new["Salinity"], color = "brown", label = 'Calculated Salinity')
    ...
    plt.title("After Outliers Removed", loc='center')
...
# Deer Island 2018
#eureka_outlier_remover("Deer_Island-SG1-2018_Annual_Data_UTC.csv", "C:\\Users\\isabe\\source\\repos\\icyeung\\SAMI_Data_SeaGrant\\Used_Data\\Salinity\\Deer_Island-SG1-2018_Annual_Data_UTC.csv")


# Deer Island 2019
eureka_outlier_remover("Deer_Island-SG1-2019_Annual_Data_UTC.csv", "C:\\Users\\isabe\\source\\repos\\icyeung\\SAMI_Data_SeaGrant\\Used_Data\\Salinity\\Deer_Island-SG1-2019_Annual_Data.csv")


# Test
#eureka_outlier_remover("test_outlier_remover.csv", "C:\\Users\\isabe\\source\\repos\\icyeung\\SAMI_Data_SeaGrant\\Eureka\\test.csv")
